...
func codexReasoningReplayScopeFromRequest(ctx context.Context, from sdktranslator.Format, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, body []byte) codexReasoningReplayScope {
	...
	if sourceFormatEqual(from, sdktranslator.FormatOpenAIResponse) {
		sessionKey = codexResponsesReplaySessionKeyFromPayload(body)
		if sessionKey == "" {
			sessionKey = codexResponsesReplaySessionKeyFromPayload(req.Payload)
		}
	}
	return codexReasoningReplayScope{
		modelName:          modelName,
...
func cacheCodexReasoningReplayFromTerminal(from sdktranslator.Format, modelName string, scope codexReasoningReplayScope, completedData []byte) {
	cacheCodexReasoningReplayFromCompleted(scope, completedData)
	if sourceFormatEqual(from, sdktranslator.FormatOpenAIResponse) {
		cacheCodexResponsesReplayFromCompleted(modelName, completedData)
	}
}
